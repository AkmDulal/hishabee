    ...
    <>
      {products?.map((product: any, i: number) => (
        <div key={i} className="relative flex flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-md">
          ...
          <div className="p-6">
            <div className="mb-2 flex items-center justify-between h-20">
              <Link href={`/products/${product?.id}`}>
                <a>
                  <p className="block font-sans text-base font-medium leading-relaxed text-blue-gray-900 antialiased">
                    {product.title}
                    <span className="block font-sans text-base font-medium leading-relaxed text-blue-gray-900 antialiased">
